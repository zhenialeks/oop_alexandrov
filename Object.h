//
// Created by george on 16.03.2022.
//
...
public:
    ...
    ~PushTask() override{
        if (!has_been_run)
            delete dynamic_cast<CountObjTask *>(task);
    };
...
public:
    ...
    CalcTask(std::string const &name, op_type action,
             HasResTask *task_1, HasResTask *task_2):Named(name), action(action), task_1(task_1), task_2(task_2){}
